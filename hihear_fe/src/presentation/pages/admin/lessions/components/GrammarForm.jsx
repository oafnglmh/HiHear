...
export default function GrammarForm({ examples, setExamples }) {

  const handleAdd = () =>
    setExamples([
      ...examples,
      { id: Date.now(), grammarRule: "", example: "", meaning: "" }
    ]);
    ...
    <div className="grammar-container">
      ...
      {examples.map((ex) => (
        <div key={ex.id} className="example-item">
          <input
            placeholder="Nội dung ngữ pháp (grammarRule)"
            value={ex.grammarRule}
            onChange={(e) => handleChange(ex.id, "grammarRule", e.target.value)}
          />
          <input
